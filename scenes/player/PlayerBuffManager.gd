...
func apply_attack_speed_boost(multiplier: float, duration: float) -> void:
	...
	if _active_timed_buffs.has("attack_speed_boost"):
		base_interval = float(_active_timed_buffs["attack_speed_boost"].original_value)
		auto_attack.attack_interval = base_interval
	else:
		base_interval = float(auto_attack.attack_interval)

	_active_timed_buffs["attack_speed_boost"] = {
		"time_left": duration,
		"duration": duration,
		"original_value": base_interval
	}
	auto_attack.attack_interval = base_interval / multiplier
	buff_started.emit("attack_speed_boost", duration)
