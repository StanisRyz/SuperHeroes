...
static func apply_meta_progression(meta_manager: Node, player: Node, auto_attack: Node, _ability_manager: Node) -> void:
	if meta_manager == null or not meta_manager.has_method("get_meta_upgrade_level"):
		return

	if player != null and is_instance_valid(player):
		var hp_level: int = meta_manager.get_meta_upgrade_level("meta_max_health")
		if hp_level > 0:
		...
		var speed_level: int = meta_manager.get_meta_upgrade_level("meta_move_speed")
		if speed_level > 0:
			var new_speed := float(player.get("speed") or 260.0) + float(speed_level) * 3.0
			player.set("speed", new_speed)

		var radius_level: int = meta_manager.get_meta_upgrade_level("meta_pickup_radius")
	...
	if auto_attack != null and is_instance_valid(auto_attack):
		var damage_level: int = meta_manager.get_meta_upgrade_level("meta_attack_damage")
		if damage_level > 0:
