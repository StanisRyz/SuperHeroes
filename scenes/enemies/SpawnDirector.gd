...
var _stage_profile_weight_bonuses: Dictionary = {
	"ranged_support":   {"shooter": 1.25, "support": 1.25, "disruptor": 1.1},
	"swarm_exploder":   {"exploder": 1.3,  "swarm": 1.3,   "splitter": 1.2},
	"defense_pressure": {"tank": 1.15, "shielded": 1.2, "splitter": 1.1},
	"portal_pressure":  {"runner": 1.2, "exploder": 1.15, "disruptor": 1.1},
}
...
const WAVE_PACKAGES: Array = [
	...
	},
	{
		"id": "charger_rush",
		"role": "hunter",
		"variant_pool": ["charger"],
		"min_count": 2, "max_count": 3,
		"weight": 0.8,
		"unlock_time": 60.0,
		"profile_bonus": {},
		"phase_weights": {"early": 0.0, "build": 1.2, "pressure": 1.3, "danger": 0.9, "pre_boss": 0.6},
		"budget_cost": 2.5,
		"min_phase": "build",
		"max_phase": "",
		"warning_level": 1,
		"warning_text": "Charger pack!",
		"package_cooldown": 16.0,
	},
	{
		"id": "splitter_wave",
		"role": "swarmer",
		"variant_pool": ["splitter"],
		"min_count": 2, "max_count": 3,
		"weight": 0.7,
		"unlock_time": 240.0,
		"profile_bonus": {"swarm_exploder": 1.3},
		"phase_weights": {"early": 0.0, "build": 0.0, "pressure": 0.3, "danger": 1.3, "pre_boss": 1.2},
		"budget_cost": 3.0,
		"min_phase": "danger",
		"max_phase": "",
		"warning_level": 2,
		"warning_text": "Splitter swarm!",
		"package_cooldown": 22.0,
	},
	{
		"id": "disruptor_squad",
		"role": "disruptor",
		"variant_pool": ["disruptor"],
		"min_count": 1, "max_count": 2,
		"weight": 0.65,
		"unlock_time": 300.0,
		"profile_bonus": {"ranged_support": 1.2},
		"phase_weights": {"early": 0.0, "build": 0.0, "pressure": 0.0, "danger": 1.1, "pre_boss": 1.4},
		"budget_cost": 2.5,
		"min_phase": "danger",
		"max_phase": "",
		"warning_level": 1,
		"warning_text": "Disruptors incoming",
		"package_cooldown": 20.0,
	},
	{
		"id": "chaos_wave",
		"role": "mixed",
		"variant_pool": ["charger", "disruptor", "splitter"],
		"min_count": 2, "max_count": 3,
		"weight": 0.6,
		"unlock_time": 300.0,
		"profile_bonus": {},
		"phase_weights": {"early": 0.0, "build": 0.0, "pressure": 0.0, "danger": 0.4, "pre_boss": 1.6},
		"budget_cost": 3.5,
		"min_phase": "pre_boss",
		"max_phase": "",
		"warning_level": 2,
		"warning_text": "Chaos wave!",
		"package_cooldown": 28.0,
	},
]
...
func _get_available_variants(seconds: float) -> Array[Dictionary]:
	var variants: Array[Dictionary] = [
		{
			...
			"display_name": "Grunt",
			"role": "swarmer",
			"role_display_name": "Grunt",
			"threat_level": 1,
			"counterplay_hint": "Easy target; manageable in small groups.",
			"behavior_id": "chase",
			"speed": 120.0,
			"max_health": 20,
			"contact_damage": 10,
	...
	if seconds >= 30.0:
		variants.append({
			...
			"display_name": "Runner",
			"role": "hunter",
			"role_display_name": "Runner",
			"threat_level": 2,
			"counterplay_hint": "Keep moving; fragile but fast.",
			"behavior_id": "chase",
			"speed": 175.0,
			"max_health": 14,
			"contact_damage": 10,
	...
	if seconds >= 45.0:
		variants.append({
			...
			"role": "hunter",
			"role_display_name": "Charger",
			"threat_level": 2,
			"counterplay_hint": "Sidestep the charge; vulnerable during cooldown.",
			"behavior_id": "charger",
			"speed": 150.0,
			"max_health": 32,
			"contact_damage": 13,
	...
	if seconds >= 60.0:
		variants.append({
			...
			"display_name": "Tank",
			"role": "bruiser",
			"role_display_name": "Tank",
			"threat_level": 3,
			"counterplay_hint": "High HP, slow; kite around it.",
			"behavior_id": "chase",
			"speed": 85.0,
			"max_health": 55,
			"contact_damage": 18,
	...
	if seconds >= 75.0:
		variants.append({
			...
			"role": "shooter",
			"role_display_name": "Shooter",
			"threat_level": 3,
			"counterplay_hint": "Break line of sight or close the gap fast.",
			"behavior_id": "shooter",
			"speed": 90.0,
			"max_health": 24,
			"contact_damage": 9,
	...
	if seconds >= 120.0:
		variants.append({
			...
			"role": "disruptor",
			"role_display_name": "Exploder",
			"threat_level": 4,
			"counterplay_hint": "Do not let it reach you; move away from the pulse.",
	...
	if seconds >= 150.0:
		variants.append({
			...
			"display_name": "Swarm",
			"role": "swarmer",
			"role_display_name": "Swarm",
			"threat_level": 2,
			"counterplay_hint": "AoE or keep moving; dangerous in numbers.",
			"behavior_id": "swarm",
			"speed": 205.0,
			"max_health": 12,
			"contact_damage": 7,
	...
	if seconds >= 180.0:
		variants.append({
			...
			"role": "bruiser",
			"role_display_name": "Shielded",
			"threat_level": 3,
			"counterplay_hint": "Burst through shield or use AoE; shield doesn't last.",
			"behavior_id": "chase",
			"speed": 105.0,
			"max_health": 42,
			"contact_damage": 14,
	...
	if seconds >= 210.0:
		variants.append({
			...
			"role": "disruptor",
			"role_display_name": "Support",
			"threat_level": 4,
			"counterplay_hint": "Kill it before it buffs the wave.",
			"behavior_id": "support",
			"speed": 82.0,
			"max_health": 34,
			"contact_damage": 7,
	...
	if seconds >= 240.0:
		variants.append({
			...
			"role": "swarmer",
			"role_display_name": "Splitter",
			"threat_level": 3,
			"counterplay_hint": "Kill before it splits; children swarm you.",
			"behavior_id": "chase",
			"speed": 100.0,
			"max_health": 35,
			"contact_damage": 12,
			"experience_value": 2,
			"body_color": Color(0.72, 0.95, 0.22, 1.0),
			"core_color": Color(0.28, 0.42, 0.06, 1.0),
			"split_on_death": true,
			"split_variant_id": "grunt",
			"split_count": 2,
			"weight": clampf((seconds - 240.0) / 160.0, 0.1, 0.65),
		})
	...
	if seconds >= 300.0:
		variants.append({
			...
			"role": "disruptor",
			"role_display_name": "Disruptor",
			"threat_level": 3,
			"counterplay_hint": "Stay out of its pulse radius or prioritize it.",
			"behavior_id": "disruptor",
			"speed": 115.0,
			"max_health": 28,
			"contact_damage": 8,
			"experience_value": 2,
			"body_color": Color(0.25, 0.88, 1.0, 1.0),
			"core_color": Color(0.04, 0.3, 0.45, 1.0),
			"disrupt_radius": 200.0,
			"disrupt_interval": 3.0,
			"disrupt_damage": 10,
			"disrupt_standoff": 140.0,
			"weight": clampf((seconds - 300.0) / 180.0, 0.1, 0.6),
		})
...
func _get_modified_weight(variant: Dictionary) -> float:
	...
	for mod_id in active_event_modifiers:
		...
		if mod.get("boost_tank_weight", false) and variant_id == "tank":
			weight *= 3.0
		if mod.get("boost_special_weight", false) and ["charger", "shooter", "exploder", "swarm", "shielded", "support", "splitter", "disruptor"].has(variant_id):
			weight *= 1.5
