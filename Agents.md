...
- Training purchase validation must verify node existence, selected-hero ownership, unlocked hero state, max level, and available Training currency. A purchase spends only Training currency, increments only that hero's node level, emits the existing `meta_upgrade_changed` signal for compatibility, and saves.
...
- `get_training_stat_modifiers_for_hero(hero_id)` returns only `hero_stats` effects for the selected hero: `max_health`, `base_damage`, and `damage_reduction`. It ignores other heroes and invalid saved ids. `damage_reduction` is capped at `TRAINING_DAMAGE_REDUCTION_CAP` (0.50).
...
- **Ability/Passive Training formatting**: `format_training_node_modifier(node: Dictionary, value: float) -> String` in MetaProgressionManager generates ability- and passive-specific player-facing text using node `tags`. Ability tags: `"solar_beam"` → "+N Solar Beam Damage"; `"frost_breath"` → "+N% Ice Breath Slow"; `"smoke_screen"` → "+N% Smoke Screen Defense"; `"mighty_clap"` → "+N% Power Clap Knockback". Passive tags: `"solar_energy"` → "+N% Solar Energy Gain" (value * 10); `"tactical_mark"` → "+N Marked Target Damage" (flat); `"rage"` → "+N% Rage Gain" (value * 10). Falls back to `format_training_modifier` for unrecognized combos. `MetaUpgradeShop._format_training_row_effect` calls `format_training_node_modifier` when available.
...
- The current `MetaUpgradeShop` Training tab remains a flat selected-hero list. Do not add full category UI, respec/reset, new currencies, gacha, crafting, random affixes, stage/zone selection, item economy changes, Ultimate Training, or deep evolution-effect integration unless explicitly requested.
