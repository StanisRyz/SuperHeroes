...
- **Ability Training formatting**: `format_training_node_modifier(node: Dictionary, value: float) -> String` in MetaProgressionManager generates ability-specific player-facing text using node `tags` (e.g. `"solar_beam"` → "+N Solar Beam Damage"; `"frost_breath"` → "+N% Ice Breath Slow"; `"smoke_screen"` → "+N% Smoke Screen Defense"; `"mighty_clap"` → "+N% Power Clap Knockback"). Falls back to `format_training_modifier` for unrecognized combos. `MetaUpgradeShop._format_training_row_effect` calls `format_training_node_modifier` when available, passing the full node `def`.
- Runtime integration point: `Arena._apply_meta_progression()` calls `MetaApplier.apply_meta_progression(meta_manager, player, auto_attack, ability_manager, hero_id)` after `HeroApplier.apply_hero()`. `MetaApplier` applies selected-hero Training stats then ability Training before shared equipment/set modifiers. Max HP updates `Player.max_health/current_health`; Base Damage adds to `PlayerAutoAttack.attack_damage` and all active damage properties; Damage Reduction adds to `Player.damage_reduction` with a 50% cap; ability Training adds only to specific mapped properties.
- Debug helpers: `debug_get_training_modifier_summary(hero_id)` reports purchased Training nodes, selected-hero stat aggregates, all effect aggregates, and ignored invalid saved node ids. `debug_get_training_ability_summary(hero_id)` reports purchased ability nodes, aggregated ability modifiers by target, and ignored invalid node ids.
...
- The current `MetaUpgradeShop` Training tab remains a flat selected-hero list. Do not add full category UI, respec/reset, new currencies, gacha, crafting, random affixes, stage/zone selection, item economy changes, passive/ultimate Training, or deep passive-effect integration unless explicitly requested.
