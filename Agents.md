...
Stage 1.7.2 marks only Worldbreaker as implemented and reward-offerable. `KnightAbilityManager3D.apply_evolution()` activates it run-locally and rejects placeholder IDs. Worldbreaker replaces Rage Wave's standard impact with three independent, pausable-manager pulses at the original cast origin: 0.0s (1.0× radius, 1.5× base damage, 7.0 knockback), 0.22s (1.45×, 1.25×, 8.5), and 0.44s (1.9×, 1.0×, 10.0). Pulses reuse Rage scaling and upgrades, refresh a 0.40 speed slow for 2.5 seconds, and render `WorldbreakerPulseEffect3D`; they do not retain the ability action token and are cleared on stop/scene cleanup. Rampage Impact and Meteor Crash remain prerequisite-visible placeholders for 1.7.3 and 1.7.4.
