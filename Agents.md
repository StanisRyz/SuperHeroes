...
- Runtime integration point: `Arena._apply_meta_progression()` calls `MetaApplier.apply_meta_progression(meta_manager, player, auto_attack, ability_manager, hero_id)` after `HeroApplier.apply_hero()`. `MetaApplier` applies selected-hero Training stats before shared equipment/set modifiers. Max HP updates `Player.max_health/current_health`; Base Damage adds to `PlayerAutoAttack.attack_damage` and active damage properties; Damage Reduction adds to `Player.damage_reduction` with a 50% cap. `Player.take_damage()` also clamps damage reduction to 50% and keeps at least 1 final damage for positive incoming hits.
- Selected hero isolation rule: Training modifiers must be read with the run's selected `hero_id` only. Guardian Training cannot affect Blaster or Vanguard runs, while shared equipment/set bonuses continue to apply to every hero.
- Debug helper: `debug_get_training_modifier_summary(hero_id)` reports purchased Training nodes, selected-hero stat aggregates, all effect aggregates, and ignored invalid saved node ids.
...
- The current `MetaUpgradeShop` Training tab remains a flat selected-hero list. Do not add full category UI, respec/reset, new currencies, gacha, crafting, random affixes, stage/zone selection, item economy changes, or deep ability-effect integration unless explicitly requested.
