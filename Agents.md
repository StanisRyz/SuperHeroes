...
Stage 1.7.3 marks Rampage Impact as implemented and reward-offerable with Sword Knockback, Furious Edge, and Heavy Bash prerequisites unchanged. It replaces standard Shield Bash with a primary cone of 1.75× Bash damage, 1.35× range, 1.35× full angle capped at 120°, 1.80× knockback, and a 0.35 speed stagger for 1.2s. Its stored-origin/direction second cone follows at 0.28s with 1.00× Bash damage, 0.85× primary range, the same full angle, and 1.25× Bash knockback. `KnightAbilityManager3D` uses one deterministic, pausable evolution-impact queue for Worldbreaker pulses and Rampage's second impact; it resolves ready events in sequence order and clears them on stop/scene cleanup without retaining action tokens. `RampageImpactEffect3D` is presentation-only and uses exact gameplay range/angle. Worldbreaker values remain unchanged; Meteor Crash remains a prerequisite-visible placeholder for 1.7.4.
