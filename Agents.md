...
- `scenes/player/Player.gd` - movement, bounds clamp, health state, XP gain multiplier.
...
- `scenes/training/CharacterTrainingDataProvider.gd` - definitions-only character Training node catalog. Owns hero-specific Training nodes and validation helpers. Required node schema: `id`, `hero_id`, `name`, `category`, `max_level`, `cost_base`, `cost_growth`, `effect_type`, `effect_per_level`, `target`, and `tags`; optional fields include `short_description` and `sort_order`. Public API: `get_all_training_nodes()`, `get_training_nodes_for_hero(hero_id)`, `get_training_nodes_for_category(hero_id, category)`, `get_training_node(node_id)`, `get_categories()`, `is_valid_training_node(node_id)`, `is_training_node_for_hero(node_id, hero_id)`, and `debug_get_training_node_summary()`. Categories are `stats`, `autoattack`, `ability_1`, `ability_2`, `ability_3`, and `passive`. Every node must have a non-empty `hero_id`; no global/shared Training nodes are allowed.
...
- `scenes/meta/MetaProgressionManager.gd` - owns soft currency, character Training levels, fixed per-hero equipment levels/upgrades, hero unlock state, lifetime stats, and save/load to user://superheroes_meta_progress.json. Calculates and applies run rewards. Never called directly by Arena. Instantiates `_training_provider` from `CharacterTrainingDataProvider.gd` and `_equipment_provider` from `EquipmentDataProvider.gd`. Training definitions come from the provider; saved Training progress remains in `training_by_hero`. Hero-specific Training API: `get_training_definitions_for_hero(hero_id)`, `get_training_definition(hero_id, node_id)`, `get_training_categories()`, `get_training_level(hero_id, node_id)`, `set_training_level(hero_id, node_id, level)`, `get_training_upgrade_cost(hero_id, node_id)`, `can_purchase_training_upgrade(hero_id, node_id)`, `purchase_training_upgrade(hero_id, node_id)`, `get_training_effect_modifiers_for_hero(hero_id)`, `get_training_effect_summary_for_hero(hero_id)`, and `debug_get_character_training_summary()`. Compatibility wrappers remain for old callers: `get_meta_upgrade_definitions()`, `get_meta_upgrade_definition(upgrade_id)`, `get_meta_upgrade_level(upgrade_id)`, `get_meta_upgrade_cost(upgrade_id)`, `can_buy_meta_upgrade(upgrade_id)`, and `buy_meta_upgrade(upgrade_id)`. Purchase rules: node must exist, belong to the selected hero, hero must be unlocked, level must be below max, existing Training currency must be sufficient, purchase increases only that hero's node level, then saves. Unknown old node ids may remain in save but are ignored by validation and purchase APIs. Equipment is still global/shared and must not be converted to character-specific Training state. Read-only inventory helpers: `get_item_template_for_instance(hero_id, instance_id) -> Dictionary`, `get_equipped_instance_id_for_slot(hero_id, slot_id) -> String`, `get_item_stat_total(hero_id, instance_id) -> Dictionary`, `debug_get_item_template_summary() -> Dictionary`. Save version is currently `SAVE_VERSION = 7`. `_clear_static_inventory_if_needed(data)` clears inventory_by_hero, equipped_by_hero, and resets equipment_by_hero levels to 0 for all heroes — runs once per save via `inventory_static_items_cleared` flag (idempotent). Called from `load_progress()` on every code path including fresh starts. `can_unequip_slot(hero_id, slot_id) -> bool` returns true if a slot has an item equipped. `unequip_slot(hero_id, slot_id) -> bool` erases the slot from equipped_by_hero (does not delete the inventory instance), emits `equipment_changed(hero_id, slot_id, "")` and `inventory_changed(hero_id)`, then saves. Empty `equipped_by_hero` is valid — `get_equipment_stat_modifiers_for_hero` returns an empty dict safely and MetaApplier's `_apply_equipment_modifiers` exits early.
- `scenes/meta/MetaApplier.gd` - static helper; applies purchased Training bonuses and supported equipment bonuses to Player, AutoAttack, and AbilityManager. Called by Arena after HeroApplier at run start. Apply order: (1) base-stat Training (`_apply_training_stat_modifiers`), (2) ability Training (`_apply_training_ability_modifiers`), (3) legacy meta_max_health / meta_attack_damage hooks, (4) equipment/set modifiers. `_apply_training_ability_modifiers` calls `meta_manager.get_training_ability_modifiers_for_hero(hero_id)` and applies target/effect pairs via `_get_ability_training_property_map(hero_id)`. Each property map entry specifies mode (`add`, `subtract_clamped`, `add_clamped`, `multiply_by_factor`) and property names. Missing properties are skipped safely. No double-counting with base-stat nodes: base-stat Training applies to all active abilities generically; ability Training adds only to the specific mapped property.
...
- Equipment is a shared global loadout. Training is per character.
- `CharacterTrainingDataProvider` owns definitions only; it must not read or write save data.
- `MetaProgressionManager` owns levels only, stored under `training_by_hero`, and must not hardcode the canonical Training catalog.
- Every Training node must belong to exactly one hero through a non-empty `hero_id`. Global/shared Training nodes are not allowed.
- Similar Training ideas on different heroes must use separate node ids.
- Required Training categories are `stats`, `autoattack`, `ability_1`, `ability_2`, `ability_3`, and `passive`; each hero should have at least one node in every category.
- Base stat Training nodes are hero-specific `stats` category nodes with `target: "hero_stats"`. Current starter ids are Guardian: `guardian_radiant_vitality`, `guardian_solar_might`, `guardian_sunforged_guard`; Night Tactician: `blaster_field_conditioning`, `blaster_tactical_precision`, `blaster_smoke_discipline`; Fury Vanguard: `vanguard_battle_conditioning`, `vanguard_brutal_strength`, `vanguard_pain_tolerance`.
- Training purchase validation must verify node existence, selected-hero ownership, unlocked hero state, and available Training currency. Training levels have no maximum — purchases are always allowed while the player has sufficient currency. A purchase spends only Training currency, increments only that hero's node level, emits the existing `meta_upgrade_changed` signal for compatibility, and saves.
- Save compatibility rule: do not reset or wipe `training_by_hero`. Missing hero dictionaries are initialized safely. Unknown old node ids may remain in saved hero dictionaries, but invalid node ids must not crash UI, purchase logic, or summaries.
- `get_training_effect_modifiers_for_hero(hero_id)` returns summed values by `effect_type`; `get_training_effect_summary_for_hero(hero_id)` returns target/effect rows for future gameplay and UI consumers. These helpers are aggregation-only and should not apply combat behavior by themselves.
- `get_training_stat_modifiers_for_hero(hero_id)` returns only `hero_stats` effects for the selected hero: `max_health`, `base_damage`, and `defense` (flat int). It ignores other heroes and invalid saved ids. No cap is applied; `defense` accumulates as a flat integer reduction applied before `damage_reduction` in `Player.take_damage`.
- **Ability Training aggregation API**: `get_training_ability_modifiers_for_hero(hero_id) -> Dictionary` returns a nested dict keyed by target (`"ability_1"`, `"ability_2"`, `"ability_3"`) → effect_type → total float. Only nodes with category `ability_1/2/3` and level > 0 are included; `stats`/`autoattack`/`passive` nodes are excluded. `get_training_ability_modifiers_for_target(hero_id, target) -> Dictionary` returns the inner dict for one target only. `get_training_ability_summary_for_hero(hero_id) -> Array[Dictionary]` returns per-node detail rows (node_id, name, category, target, effect_type, level, total). `debug_get_training_ability_summary(hero_id) -> Dictionary` adds ignored-invalid-nodes tracking.
- **Ability Training property map** (in MetaApplier): `_get_ability_training_property_map(hero_id)` maps `"target|effect_type"` keys to `{mode, properties, clamp_*}` entries. Guardian: `ability_1|ability_damage` → add → `solar_beam_damage`; `ability_2|slow_strength` → subtract_clamped(min=0.1) → `frost_breath_slow_multiplier`; `ability_3|ability_damage` → add → `death_dash_damage`. Blaster: `ability_1|damage_reduction` → add_clamped(max=0.75) → `smoke_screen_damage_reduction`; `ability_2|ability_damage` → add → `explosive_trap_damage`; `ability_3|ability_damage` → add → `grappling_hook_damage`. Vanguard: `ability_1|ability_damage` → add → `rage_wave_damage`; `ability_2|knockback_power` → multiply_by_factor → `mighty_clap_knockback_force`; `ability_3|ability_damage` → add → `rage_leap_damage`. Unknown hero_id returns empty map safely.
- **Safe property application rules**: missing properties (`.get() == null`) are skipped without error. No crash if a mapped property does not exist. No double-counting: base-stat `base_damage` Training applies to all active abilities generically via `_apply_base_damage_bonus`; ability Training applies only to the specific mapped property.
- **Selected hero isolation rule**: Training modifiers must be read with the run's selected `hero_id` only. Guardian Training cannot affect Blaster or Vanguard runs; shared equipment/set bonuses continue to apply to every hero.
- **No global Training nodes**: every Training node must have a non-empty `hero_id`. Nodes from other heroes are excluded from aggregation by the provider's `get_training_nodes_for_hero` filter.
- **Training formatting rules — flat only**: All Training UI text must be flat numbers with no percent signs. `format_training_modifier(effect_type, value)` handles base stat types: `max_health` → "+N Max HP"; `base_damage` → "+N Base Damage"; `defense` → "+N Defense"; `autoattack_damage` → "+N Autoattack Damage"; `ability_damage` → "+N Ability Damage". Falls back to capitalized effect_type string for unrecognized types. `format_training_node_modifier(node: Dictionary, value: float) -> String` generates ability- and passive-specific flat text using node `tags`. Ability tags: `"solar_beam"` → "+N Solar Beam Damage"; `"frost_breath"` → "+N Ice Breath Control"; `"smoke_screen"` → "+N Smoke Shield"; `"mighty_clap"` → "+N Power Clap Force". Passive tags: `"solar_energy"` → "+N Solar Energy Flow"; `"tactical_mark"` → "+N Marked Target Damage"; `"rage"` → "+N Rage Flow". `MetaUpgradeShop._format_training_row_effect(def, level)` calls `format_training_node_modifier` for the node-specific label.
- **Passive Training aggregation API**: `get_training_passive_modifiers_for_hero(hero_id) -> Dictionary` returns flat dict of effect_type → total float (only `passive` category nodes, level > 0, selected hero only). `get_training_passive_summary_for_hero(hero_id) -> Array[Dictionary]` returns per-node detail rows. `debug_get_training_passive_summary(hero_id) -> Dictionary` adds ignored-invalid-nodes tracking. Passive aggregation excludes stats/autoattack/ability nodes.
- **Passive Training property map** (in MetaApplier): `_get_passive_training_property_map(hero_id)` maps `"passive|effect_type"` keys to `{mode, scale, properties}` entries. Modes: `multiply_by_factor_scaled` → `current * (1 + value * scale)`; `add_scaled` → add `value * scale`. Guardian: `passive|passive_gain` → multiply_by_factor_scaled(0.10) → `solar_energy_per_second`. Blaster: `passive|mark_damage` → add_scaled(0.10) → `tactical_mark_autoattack_damage_multiplier`. Vanguard: `passive|rage_gain` → multiply_by_factor_scaled(0.10) → `["rage_per_damage_taken", "rage_per_damage_dealt", "rage_per_hit"]`. Unknown hero_id returns empty map safely.
- Runtime integration point: `Arena._apply_meta_progression()` calls `MetaApplier.apply_meta_progression(meta_manager, player, auto_attack, ability_manager, hero_id)` after `HeroApplier.apply_hero()`. Apply order: (1) stat Training, (2) ability Training, (3) passive Training, (4) legacy meta hooks, (5) equipment/set modifiers. Max HP updates `Player.max_health/current_health`; Base Damage adds to `PlayerAutoAttack.attack_damage` and all active damage properties; Defense Training adds flat `Player.defense` (no cap); ability Training adds only to specific mapped properties; passive Training modifies signature mechanic properties only for the selected hero. `Player.take_damage` applies defense first (`amount = max(amount - defense, 1)`), then `damage_reduction` (clamped to 50%) from non-Training sources (e.g. Smoke Screen), then shield consumption.
- **No double-counting**: passive nodes are excluded from ability aggregation (different category filter); ability nodes are excluded from passive aggregation. Base-stat `base_damage` Training applies to all active damage props but not to passive mechanic properties.
- Debug helpers: `debug_get_training_modifier_summary(hero_id)` — stat aggregates; `debug_get_training_ability_summary(hero_id)` — ability nodes; `debug_get_training_passive_summary(hero_id)` — passive nodes, aggregated passive modifiers, ignored invalid ids.
- Compatibility wrappers (`get_meta_upgrade_definitions`, `get_meta_upgrade_definition`, `get_meta_upgrade_level`, `get_meta_upgrade_cost`, `can_buy_meta_upgrade`, `buy_meta_upgrade`) exist for older callers. New Training work should use the hero-specific API.
- **Training UI layout**: `MetaUpgradeShop` Training screen uses a two-panel HBoxContainer. Left panel (220px fixed): hero color swatch (204×204 ColorRect from `HeroDataProvider.get_hero().color`), hero name label, stat preview label ("Training Bonus: HP +N / Attack +N / Defense +N" from `get_character_training_stat_preview`), and summary label ("Training Level: N / Stats: N   Abilities: N" from `get_training_progress_summary_for_hero`). Right panel (expand fill): two tab buttons ("Stats" and "Abilities"), category header label, and a ScrollContainer containing `_training_cards_vbox`.
- **Training UI tab grouping**: Tab "Stats" covers categories `["stats", "autoattack"]` with subheaders "Base Stats" and "Autoattack". Tab "Abilities" covers categories `["ability_1", "ability_2", "ability_3", "passive"]` with subheaders "Ability 1", "Ability 2", "Ability 3", "Passive". `get_training_definitions_for_ui_tab(hero_id, tab_id)` returns nodes in that tab's categories in definition order. `_selected_training_tab: String` holds "stats" or "abilities". `_training_tab_buttons: Dictionary` maps tab_id → Button.
- **Training UI card rules**: Cards show "Lv X" only — no max level cap, no MAX state ever. Cost formula: `round(cost_base * pow(cost_growth, current_level))` capped at 999,999,999. Upgrade button is always labeled "Upgrade" (disabled if insufficient currency). Left panel stat preview refreshes after every successful purchase via `_refresh_training_hero_summary()`.
- **`get_character_training_stat_preview(hero_id) -> Dictionary`**: returns `{hero_id, max_health: int, attack_damage: int, defense: int}` — Training-only contributions from `get_training_stat_modifiers_for_hero`; does not include base hero stats or equipment.
- Do not add respec/reset, new currencies, gacha, crafting, random affixes, stage/zone selection, item economy changes, unlock/tier requirements, or deep evolution-effect integration into Training unless explicitly requested.
...
- `MetaUpgradeShop` emits `equipment_buy_requested(hero_id, equipment_id)` → `Main._on_equipment_buy_requested(hero_id, equipment_id)` → `MetaProgressionManager.purchase_equipment_upgrade(hero_id, equipment_id)`.
