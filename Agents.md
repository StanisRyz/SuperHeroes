...
`EnemySpawner3D` owns safe instancing, ring-position selection, alive counting, target assignment, death wiring, and XP pickup creation. It must query the existing `SpawnDirector` for spawn interval, alive cap, variants, and wave packages; never copy its phase/weight/wave policy. `EnemyVariant3DAdapter` converts director speeds with `40 pixels = 1 world unit` and deliberately forces unsupported behavior IDs to chase while preserving their source id. `ExperiencePickup3D` is a Pickups Area3D that grants XP exactly once on Player3D overlap; it has no magnet logic. With Player3D `prototype_debug_enabled`, the existing `debug_kill_nearby_enemies` action kills the nearest prototype enemy for inspection.
