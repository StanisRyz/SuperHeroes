...
Stage 1.8.3 ports Berserker Focus. Blood Frenzy (`splash_melee_lifesteal`) is a three-level attack line adding `2 HP` per hit enemy per level and applies one combined post-swing heal. Recovery Field is a three-level passive at `4/6/8` healing, `12/10.5/9s` intervals, and `2.0/2.375/2.75` world-unit visual radii. Wide Landing (`rage_leap_radius`) is a three-level active line adding `0.55` Leap radius per level; it stacks with the existing line and therefore also expands Meteor Crash. Berserker Focus requires Blood Frenzy, Recovery Field, and Wide Landing; it uses Rage-scaled range, truncated damage, 1–4 deterministic targets, an enhanced refreshed speed buff, and its legacy interval/retry rules without generating Rage. Its HUD title replaces Battle Focus, and Evolution Reward shows passive targets and focuses the first visible option. Rage Field damage now uses legacy integer truncation.

Stage 1.8.4 implements Gravity Rage (`rage_leap_final_impact`, triple `vanguard_final_impact`) from Finishing Blow (`splash_melee_execute`), Guardian Drone (`guardian_drone`), and Leap Ready (`rage_leap_cooldown`), all at their authoritative current maximum of three levels. Finishing Blow gains thresholds `0.20 / 0.40 / 0.60` and independently applies `1.45x` damage only to enemies already at or below that health ratio, after normal Rage and Fury Combo damage calculation. Guardian Drone deals fixed `5 / 8 / 11` damage every `3.4 / 3.0 / 2.6s` to the nearest living enemy in `11.5` world units; its gold low-poly visual orbits the Knight at `1.45` units and is lifecycle-owned by PassiveAbilityManager3D. Leap Ready applies `-1.2s` cooldown and `+0.5` distance per level to all Crushing Leap movement paths, with a `3.5s` cooldown floor. Gravity Rage renames Magnet Core in the HUD, multiplies only its `45 / 85 / 125px` level bonus by `2.6` (never its `140px` base), producing evolved pickup radii `6.425 / 9.025 / 11.625`. Every `3.2s`, it uses radii `8.425 / 11.025 / 13.625` to pull enemies with force `3.0` for `0.22s` and a `0.55` movement-speed modifier for `0.8s`; it deals no damage and generates no Rage. Evolution Reward releases stale focus and assigns deferred focus to the first option or Continue. The five-level migration remains future work.

Stage 1.8.5 ports the three-level legacy Orbit Shields passive (`orbit_shields`) without introducing Solar Aegis. Its maximum charges are `1 / 1 / 2` and regeneration intervals are `18 / 14 / 12s`. Player3D now owns run-local shield charges and emits `shield_changed` plus `shield_blocked`; one charge completely blocks one eligible hit before health, `damage_taken`, Rage, hit animation, or death handling. First selection fills `1 / 1`; the level-three capacity increase fills to `2 / 2`. Missing charges regenerate sequentially through PassiveAbilityManager3D's pausable timer. OrbitShieldVisual3D is a manager-owned cyan low-poly marker pair orbiting at `1.05` units and mirrors Player3D charge changes. ShieldLabel shows the 3D charge state and regeneration detail while preserving the old 2D buff-manager path; Arena3D summaries retain charges, capacity, and blocked-hit count before cleanup. Guardian Drone now places itself immediately on setup, preventing first-frame arcs from world origin. Current maximum levels remain authoritative and the five-level migration remains future work.

Stage 1.8.6 ports Storm Relay (`storm_relay`) and Chain Lightning (`chain_lightning`) as exact three-level passive lines. Storm Relay deals fixed `8 / 12 / 16` damage every `5.5 / 4.8 / 4.2s` to the nearest living enemy in `13.0` world units; without a target it retries after `0.35s`. Chain Lightning deals fixed `6 / 9 / 12` damage per target every `6.6 / 5.8 / 5.0s`, starts within `12.5` world units, then bounces from the prior saved target position through unique enemies at `5.25 / 6.0 / 6.75` units. Its total targets are `2 / 3 / 4`, including the initial target. Both use shared pause-safe passive timers and presentation-only lightweight arcs, with neither Rage scaling nor Rage generation. Orbit Shields now publishes a throttled `0.25s` countdown refresh while regenerating so its existing HUD tooltip decreases live. Solar Storm, Shock Net, and Time Dilator remain unimplemented; current maximum levels, the future five-level migration note, and the 300-second run duration are unchanged.

Stage 1.8.7 completes the nine base legacy passive ports with Time Dilator (`time_dilator`). Its three levels pulse every `8.5 / 7.5 / 6.5s` at converted radii `4.75 / 5.5 / 6.25` world units, applying the stable Enemy3D `time_dilator` movement-speed modifier at `0.72 / 0.64 / 0.56` for `2.5 / 3.0 / 3.5s`. Each pale blue-violet pulse uses the exact gameplay radius and occurs even with no enemies; it deals no damage, generates no Rage, and adds no knockback or stun. It uses the shared pause-safe timer, generic HUD/summary integration, and normal lifecycle cleanup. Stasis Field and Crushing Storm remain unimplemented; maximum levels, the future five-level migration, and the 300-second run duration remain unchanged.

Stage 1.8.8 adds reusable attack-target evolution routing alongside active and passive routing. Earthsplitter (`rage_wave_earthsplitter`, triple `vanguard_earthsplitter`) requires Wide Fury (`splash_melee_radius`), Static Field, and Wave Reach (`rage_wave_radius`) at their current maximum levels, then evolves `splash_melee`. Wide Fury has four levels of `+0.35` world melee radius; Wave Reach has three levels of `+0.75` Rage Wave radius and `+0.04` Rage-radius scaling, using `base * (1 + (0.18 + bonus) * RageRatio)` for normal Rage Wave and Worldbreaker. After a successful normal melee impact, Earthsplitter creates a Web-compatible ground crack in the stored swing direction: `6.5` range, `1.3325` width, and `0.75x` base swing damage. It can overlap normal melee targets but adds no knockback, Fury, Blood Frenzy, Finishing Blow, or Rage. The primary attack name changes from Fury Strike to Earthsplitter and Arena3D records attack evolution IDs/counts in the run summary. Crushing Storm is the next planned attack-target evolution.

Stage 1.8.9 ports Crushing Storm (`rage_wave_crushing_storm`, triple `vanguard_crushing_storm`) through the shared attack route. It requires Berserker Frenzy (`splash_melee_frenzy`), Time Dilator, and Crushing Current (`rage_wave_deep_slow`). Berserker Frenzy is three levels of `+0.10` maximum Rage damage multiplier with a `1.95` cap. Crushing Current has three levels of `+0.8s` Rage Wave slow duration, `-0.06` slow multiplier to a `0.20` minimum, `-0.5s` cooldown to a `2.5s` minimum, and `+0.04` Rage radius scaling. While selected, normal Fury Strike gains `RageRatio * 1.35` multiplier. After each successful normal impact, pressure storm radius is `meleeRadius * lerp(1.55, 2.45, RageRatio)`, damage is `baseSwing * lerp(0.55, 1.15, RageRatio)`, slow is `lerp(0.62, 0.28, RageRatio)` for `1.25 + RageRatio`; its red-orange ring is presentation-only. The stable pressure modifier stacks with Time Dilator. Earthsplitter and Crushing Storm can coexist, with the newest selected attack evolution providing the primary-attack display name.
...
Stage 1.9.1 normalizes the Knight's 3D run progression to exactly 27 canonical lines: 9 attack, 9 passive, and 9 active. Every line has one unique category grid index, an evolution ID, and a matching evolution role; all additional run-upgrade lines were removed. The nine triples are Worldbreaker (Fury Strike Power / Orbit Shields / Wave Surge), Earthsplitter (Wide Fury / Static Field / Wave Reach), Crushing Storm (Berserker Frenzy / Time Dilator / Crushing Current), Rampage Impact (Knockback Force / Storm Relay / Clap Force), Seismic Fan (Ground Shockwave / Chain Lightning / Wide Clap), Rage Field (Fury Combo / Battle Focus / Impact Wave), Meteor Crash (Fury Tempo / Magnet Core / Leap Impact), Berserker Focus (Blood Frenzy / Recovery Field / Wide Landing), and Gravity Rage (Finishing Blow / Guardian Drone / Leap Ready). Every passive now participates in exactly one evolution. `EvolutionManager3D.get_progression_matrix_validation_errors()` verifies catalog counts, metadata, unique usage, owners, and one attack/passive/active prerequisite per evolution. Stage 1.9.2 will standardize all canonical lines to five levels.
...
Stage 2.1.1 hardens Crossbowman runtime setup: its player scene now matches Knight collision layers, Arena3D validates the configured player scene and required common nodes before starting a run, and `HeroVisual` is a stable shared visual contract. Crossbow Shot is the player-facing name for the preserved `solar_ray` corridor hit; it reports a structured shot result for later upgrades. Guardian's scope is enforced when options are created and when upgrades are applied. HUD and mobile ability controls are fully hidden until Stage 2.2 adds Guardian active abilities. Solar Energy always retains its stable name and exposes empowered state/time separately.

Stage 2.1.2 replaces the temporary Knight model with KayKit `Ranger.glb`. Crossbowman uses `Rig_Medium_CombatRanged.glb`: `Ranged_2H_Shoot` drives Crossbow Shot, with `Ranged_2H_Aiming` and `Ranged_2H_Shooting` configured for Stage 2.2 charged-shot and volley presentation only. `crossbow_2handed.gltf` attaches at Ranger `handslot.r`; its attachment owns the animated muzzle marker. The neutral `KayKitAnimatedVisual`/`CharacterVisualController` contract remains shared with Knight, while Crossbowman configures its own model, weapon, and clips. Crossbow Shot damage, range, interval, and hit rules are unchanged; its bounded bolt now spawns at the ranged-animation impact.
