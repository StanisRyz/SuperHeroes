...
`Arena3D` owns Player3D spawning, playable bounds, and camera wiring. [`Player3D.gd`](scenes3d/player/Player3D.gd) remains a reusable `CharacterBody3D` controller for health, XP, input, mobile movement, dash, and facing; [`KnightVisual.tscn`](scenes3d/player/visuals/KnightVisual.tscn) owns the KayKit model, animation import, and sword/shield presentation. The Knight model (`Characters/gltf/Knight.glb`) uses the `Rig_Medium` rig. Its Idle_A/Running_A locomotion and Use_Item/Hit_A/Death_A one-shots come from the matching KayKit animation GLBs. `Use_Item` is the current attack animation mapping because this asset set has no clip literally named Attack.

KnightVisual attaches `sword_1handed.gltf` to `handslot.r` and `shield_round.gltf` to `handslot.l`. It emits one `attack_impact` at the configured normalized point in Use_Item; this is presentation timing only, reserved for stage 1.5 melee damage. The model's +Z-facing visor is corrected inside KnightVisual's ModelOffset, so Player3D never contains KayKit paths, animation names, or bone names. If Godot has not imported the new KayKit GLB/glTF files yet, open the project once and allow its normal asset import to finish before running the isolated scene.
...
`EnemySpawner3D` reuses the current `SpawnDirector` for spawn intervals, alive caps, variants, and wave packages. `EnemyVariant3DAdapter` preserves the director dictionary values but converts legacy speeds with the explicit rule `40 2D pixels = 1 world unit` (120 pixels/sec becomes 3 world units/sec). Unsupported behavior IDs retain their source value for diagnostics but run as chase only. Defeated enemies immediately create an `ExperiencePickup3D`, which grants its configured XP value on Player3D overlap; magnet behavior, navigation/pathfinding, special roles, elite/miniboss/boss logic, and player attack damage remain future work.
